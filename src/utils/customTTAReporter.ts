...
class CustomTTAReporter implements Reporter {
  ...
  private reporterOptions: Record<string, any> = {};

  constructor(reporterOptions: Record<string, any> = {}) {
    this.reporterOptions = reporterOptions;
    logger.info('📦 TTA Reporter received options:', this.reporterOptions);
  }
  ...
  onBegin(config: FullConfig, suite: Suite): void {
    ...
    this.outputFile =
      ...
      path.join(PROJECT_ROOT, 'artifacts', 'reports', 'tta-report', `report_${this.runId}.html`); // Default to `artifacts/reports/tta-report/report_<timestamp>.html` if not specified, to avoid overwriting previous reports
    this.config = config;
  ...
  private initializeLiveReport(): void {
    ...
    logger.info(`📡 Real-time report: ${this.outputFile}`);
  }

  private logBox(lines: string[]): void {
    // Simple, clean banner without box-drawing characters to avoid distorted output.
    const strs = lines.map((l) => String(l));

    // Respect terminal width for truncation but do NOT draw boxes or separators.
    const terminalCols =
      typeof process !== 'undefined' && process.stdout && typeof process.stdout.columns === 'number'
        ? process.stdout.columns
        : 80;
    const maxInnerAllowed = Math.max(40, Math.min(120, terminalCols - 6));

    const displayLines = strs.map((s) => {
      const chars = [...s];
      if (chars.length > maxInnerAllowed) {
        return chars.slice(0, maxInnerAllowed - 3).join('') + '...';
      }
      return s;
    });
  ...
  onStepEnd(test: TestCase, _result: TestResult, step: TestStep): void {
    if (step.category === 'test.step') {
      ...
      logger.info(`   ${status} ${step.title} ${duration}`);
  ...
  onTestEnd(test: TestCase, result: TestResult): void {
    ...
    const lines: string[] = [];
    lines.push(`${statusIcon} RESULT: ${status.toUpperCase()} | Duration: ${testTime}`);
    if (result.error) {
      lines.push(`⚠️ Error: ${result.error.message?.substring(0, 200)}`);
    }
    lines.push(
    ...
    for (const step of currentTestSteps) {
      for (const [name, screenshotPath] of stepScreenshots) {
        ...
        if (
          cleanedName &&
          (titleLower.includes(cleanedName) || cleanedName.includes(titleLower.substring(0, 20)))
        ) {
          step.screenshot = screenshotPath;
          break;
        }
  ...
  async onEnd(_result: FullResult): Promise<void> {
    ...
    this.logBox([
      '📊 FINAL TEST SUMMARY',
      `📊 Total: ${String(this.suiteStats.total)}`,
      `⏱️ Duration: ${duration}`,
      `📈 Pass Rate: ${passRate}%`,
    ]);
    ...
    logger.info(`✅ Report generated: ${this.outputFile}`);
  }
  ...
  private generateTestTable(): string {
    ...
    for (const test of this.testResults) {
      ...
      const firstScreenshot =
        test.screenshots[0]?.path || test.steps.find((s) => s.screenshot)?.screenshot || '';
